"""
Cell Detection and Tracking System for two orgaoind circles.
--------------------------------------------
"""
...
class Config:
    ...
    ROI_PADDING = 50  # pixels from each edge
    
    # Video Processing
    PROCESSING_DURATION = 712  # 712 seconds
    VIDEO_FPS = 24
    SLOW_MOTION_FACTOR = 0.4  
    
    # Object Detection
    SINGLE_CELL_THRESHOLD = 30
    CLUSTER_THRESHOLD = 210
    BLUR_LEVEL = 5
    BRIGHTNESS_THRESHOLD = 165
    CIRCULARITY_THRESHOLD = 0.12
    ...
    # Image Processing
    KERNEL_SIZE = (5, 5)
    MIN_CONTRAST_VALUE = 164
    MAX_CONTRAST_VALUE = 196
    BRIDGE_WIDTH = 11
    ...
    # Grid Video Display Duration
    GRID_DISPLAY_SECONDS = 1.0
...
class CellTracker:
    ...
    def _create_visualization(self, frame, tracked_objects):
        ...
        if tracked_objects and len(tracked_objects) > 0:
            ...
            # Round to nearest period of 120
            current_frame = (current_frame // 120) * 120
    ...
    def _get_near_zone_thresholds(self, frame_number):
        """Calculate dynamic near zone thresholds based on circle radii."""
        ...
        # Calculate thresholds based on respective radii
        threshold1 = circles[0][2] * Config.CIRCLE_RADIUS_MULTIPLIER  # For smaller circle
        threshold2 = circles[1][2] * Config.CIRCLE_RADIUS_MULTIPLIER  # For larger circle
        
        return threshold1, threshold2

    def _classify_grid_zones(self, frame_number):
        """Classify grid cells into zones based on organoid circle positions with dynamic thresholds."""
        # Get grid dimensions
        ...
        # Get organoid circles for current frame
        circles = self._interpolate_organoid_circles(frame_number)
        if not circles or len(circles) < 2:
            return zones
        
        # Sort circles by radius
        circles.sort(key=lambda x: x[2])  # Sort by radius
        ...
        near_threshold1, near_threshold2 = self._get_near_zone_thresholds(frame_number)
        if near_threshold1 is None or near_threshold2 is None:
            return zones
            
        # Calculate line between circles
        c1 = np.array([roi_circles[0][0], roi_circles[0][1]])  # Smaller circle
        c2 = np.array([roi_circles[1][0], roi_circles[1][1]])  # Larger circle
        circle_vector = c2 - c1
        circle_distance = np.linalg.norm(circle_vector)
        
        if circle_distance == 0:
            return zones
            
        # Calculate unit vectors for direction and perpendicular
        circle_direction = circle_vector / circle_distance
        perpendicular_direction = np.array([-circle_direction[1], circle_direction[0]])
        
        # Calculate midpoint between circles
        midpoint = (c1 + c2) / 2
        
        # Define width of "between" zone
        between_width = min(near_threshold1, near_threshold2) * 0.8
        ...
        for y in range(Config.GRID_ROWS):
            for x in range(Config.GRID_COLS):
                ...
                # Calculate distances to circles
                dist_to_c1 = np.linalg.norm(cell_point - c1)
                dist_to_c2 = np.linalg.norm(cell_point - c2)
                
                # Vector from midpoint to cell
                to_cell = cell_point - midpoint
                
                # Project onto circle direction to check if between circles
                along_direction = np.dot(cell_point - c1, circle_direction)
                projection_ratio = along_direction / circle_distance
                
                # Calculate perpendicular distance to the line between circles
                perp_distance = abs(np.dot(to_cell, perpendicular_direction))
                
                # Check zones using dynamic thresholds
                if dist_to_c1 < near_threshold1:
                    zones[y, x] = 1  # Near circle 1 (smaller)
                elif dist_to_c2 < near_threshold2:
                    zones[y, x] = 2  # Near circle 2 (larger)
                # Check if point is in between zone
                elif (0 <= projection_ratio <= 1) and (perp_distance < between_width):
                    zones[y, x] = 3  # Between circles
                else:
                    # Determine which side of the perpendicular line the point is on
                    side = np.dot(to_cell, circle_direction)
                    zones[y, x] = 4 if side < 0 else 5  # 4 for left (circle 1), 5 for right (circle 2)
        
        # Print statistics
        unique, counts = np.unique(zones, return_counts=True)
        zone_counts = dict(zip(unique, counts))
        print("\nZone classification statistics:")
        print(f"Near circle 1 (1): {zone_counts.get(1, 0)} cells (threshold: {near_threshold1:.1f}px)")
        print(f"Near circle 2 (2): {zone_counts.get(2, 0)} cells (threshold: {near_threshold2:.1f}px)")
        print(f"Between circles (3): {zone_counts.get(3, 0)} cells")
        print(f"Far area circle 1 (4): {zone_counts.get(4, 0)} cells")
        print(f"Far area circle 2 (5): {zone_counts.get(5, 0)} cells")
    ...
    def _create_grid_visualization(self, frame_size, frame_number, historical_tracks):
        ...
        zone_colors = {
            1: (255, 200, 200),  # Light red for near circle 1 (smaller)
            2: (200, 200, 255),  # Light blue for near circle 2 (larger)
            3: (240, 240, 200),  # Light yellow for between circles
            4: (200, 255, 200),  # Light green for far area - circle 1 (left)
            5: (220, 255, 220),  # Slightly different green for far area - circle 2 (right)
        }
        ...
        if frame_number in self.organoid_data:
            ...
            for circle in circles:
                x, y, r = circle
                # Draw with more visible parameters
                cv2.circle(vis_frame, (int(x), int(y)), int(r), (0, 255, 0), 2)
                cv2.circle(vis_frame, (int(x), int(y)), 2, (0, 0, 255), -1)
        
        # Draw movement vectors
        vector_colors = {
            1: (200, 0, 0),    # Red for near circle 1
            2: (0, 0, 200),    # Blue for near circle 2
            3: (150, 150, 0),  # Dark yellow for between
            4: (0, 150, 0),    # Dark green for far area - circle 1
            5: (0, 180, 0)     # Dark green for far area - circle 2
        }
        
        for grid_key, data in grid_movements.items():
            movements = data['movements']
            zone = data['zone']
            if movements:  # Only draw if there are movements in this cell
                x, y = grid_key
                start_point = (
        ...
        legend_y = 60
        zone_names = {
            1: "Near Circle 1 (Small)",
            2: "Near Circle 2 (Large)",
            3: "Between Circles",
            4: "Far Area - Circle 1",
            5: "Far Area - Circle 2"
        }
        ...
        for zone, color in zone_colors.items():
            cv2.rectangle(vis_frame, (10, legend_y), (30, legend_y + 20), color, -1)
            cv2.putText(vis_frame, zone_names[zone], (40, legend_y + 15),
    ...
    def _calculate_vector_angles(self, grid_movements, frame_number):
        """Calculate angles between cell movement vectors and vectors to circle centers."""
        angles = {}
        ...
        for circle in circles:
            ...
            roi_x = x - self.roi[0]
            roi_y = y - self.roi[1]
            circle_centers.append(np.array([roi_x, roi_y]))
        ...
        for grid_key, data in grid_movements.items():
            ...
            if movements and zone != 3:  # Skip "between circles" zone
                x, y = grid_key
                ...
                # Determine which circle to reference based on zone
                circle_idx = 0 if zone in [1, 4] else 1  # First circle for zones 1,4; second for zones 2,5
                circle_center = circle_centers[circle_idx]
    ...
    def _create_angle_histogram(self, angles, frame_number):
        """Create histograms of angles for each zone and combined views."""
        # Create figure with subplots for all 6 histograms
        fig = plt.figure(figsize=(20, 15))
        zones = [1, 4, 2, 5]  # Skip zone 3 (between circles)
        zone_names = {
            1: "Near Circle 1",
            2: "Near Circle 2",
            4: "Far Area - Circle 1",
        ...
        bin_centers = bins[:-1] + 10  # For storing histogram data
        ...
        # Create combined lists for near and far
        near_angles = zone_angles[1] + zone_angles[2]
        far_angles = zone_angles[4] + zone_angles[5]
        ...
        self.histogram_data['near'].extend(near_angles)
        self.histogram_data['far'].extend(far_angles)
        self.histogram_data['zone1'].extend(zone_angles[1])
        self.histogram_data['zone2'].extend(zone_angles[2])
        self.histogram_data['zone4'].extend(zone_angles[4])
        self.histogram_data['zone5'].extend(zone_angles[5])
        
        # Create all 6 subplots
        # First row: individual zones
        ...
        # Second row: combined near and far
        # Near zones (1,2)
        plt.subplot(3, 2, 5)
        if near_angles:
            n, bins, patches = plt.hist(near_angles, bins=bins, edgecolor='black')
            for i in range(len(patches)):
                patches[i].set_facecolor(bin_colors[min(i, len(bin_colors)-1)])
            plt.title(f"Combined Near Zones (1,2) (n={len(near_angles)})")
            plt.xlabel("Angle (degrees)")
            plt.ylabel("Count")
            plt.grid(True, alpha=0.3)
        ...
        plt.subplot(3, 2, 6)
        if far_angles:
            n, bins, patches = plt.hist(far_angles, bins=bins, edgecolor='black')
            for i in range(len(patches)):
                patches[i].set_facecolor(bin_colors[min(i, len(bin_colors)-1)])
            plt.title(f"Combined Far Zones (4,5) (n={len(far_angles)})")
            plt.xlabel("Angle (degrees)")
            plt.ylabel("Count")
            plt.grid(True, alpha=0.3)
    ...
    def _create_aggregate_histogram(self):
        """Create an aggregate histogram showing all periods combined."""
        ...
        plot_data = [
            ('zone1', 1, "Near Circle 1"),
            ('zone2', 2, "Near Circle 2"),
            ('zone4', 3, "Far Area - Circle 1"),
            ('zone5', 4, "Far Area - Circle 2"),
            ('near', 5, "Combined Near Zones (1,2)"),
            ('far', 6, "Combined Far Zones (4,5)")
        ]
    ...
    def process_video(self):
        ...
        # Calculate frames per period for grid videos based on desired display duration
        grid_frames_per_period = int(Config.VIDEO_FPS * Config.GRID_DISPLAY_SECONDS)
        ...
        current_period = -1  # Track which period we're in
        grid_frames_written = 0  # Track how many frames we've written for current period
        
        print(f"\nInitializing video processing with:")
        print(f"Total frames: {frame_count}")
        print(f"Window size: {window_size} frames")
        print(f"Frames per period in grid videos: {grid_frames_per_period}")
        ...
        while window_start < frame_count:
            ...
            for idx, frame_number in enumerate(range(window_start, window_end)):
                if idx < len(window_visualizations):
                    ...
                    # Handle grid visualizations
                    period = frame_number // Config.FRAMES_PER_PERIOD
                    
                    if period != current_period:
                        # New period started, create new grid frames
                        current_period = period
                        grid_frames_written = 0
                        
                        # Create new grid visualizations
                    ...
                    # Write grid frames based on grid_frames_per_period
                    if grid_frames_written < grid_frames_per_period:
                        if last_grid_frame is not None:
                            grid_out.write(last_grid_frame)
                        if last_angle_grid_frame is not None:
                            angle_grid_out.write(last_angle_grid_frame)
                        grid_frames_written += 1
...
def main():
    ...
    video_path = "../../data/T1_Video27.mp4"  # Update with video path
